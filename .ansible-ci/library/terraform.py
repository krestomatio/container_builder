...
def _state_args(state_file):
    ...
    if state_file and not os.path.exists(state_file):
        module.fail_json(
            msg='Could not find state_file "{0}", check the path and try again.'.format(state_file))
    return []
...
def build_plan(command, project_path, variables_args, state_file, targets, state, apply_args, plan_path=None):
    ...
    plan_command.extend(['-input=false', '-no-color',
    ...
    rc, out, err = module.run_command(
        plan_command + variables_args, cwd=project_path)
    ...
    module.fail_json(
        msg='Terraform plan failed with unexpected exit code {0}. \r\nSTDOUT: {1}\r\n\r\nSTDERR: {2}'.format(rc, out, err))
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            state=dict(default='present', choices=[
                       'present', 'absent', 'planned']),
            variables=dict(type='dict'),
            variables_files=dict(
                aliases=['variables_file'], type='list', elements='path'),
    ...
    if force_init:
        if overwrite_init or not os.path.isfile(os.path.join(project_path, ".terraform", "terraform.tfstate")):
            init_plugins(command[0], project_path, backend_config,
                         backend_config_files, init_reconfigure, provider_upgrade, plugin_paths)
    ...
    if state == 'absent':
        ...
        if any([os.path.isfile(project_path + "/" + plan_file), os.path.isfile(plan_file)]):
            ...
            module.fail_json(
                msg='Could not find plan_file "{0}", check the path and try again.'.format(plan_file))
    else:
    ...
    if needs_application and not module.check_mode and state != 'planned':
        rc, out, err = module.run_command(
            command, check_rc=False, cwd=project_path)
        if rc != 0:
            if workspace_ctx["current"] != workspace:
    ...
    outputs_command = [command[0], 'output',
                       '-no-color', '-json'] + _state_args(state_file)
    ...
    module.exit_json(changed=changed, state=state, workspace=workspace,
                     outputs=outputs, stdout=out, stderr=err, command=' '.join(command))
